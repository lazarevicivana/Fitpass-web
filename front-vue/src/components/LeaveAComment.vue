...
export default {
  ...
  methods:{
    CommentSubmit(){
      this.comment.customerId = this.user.username
      this.comment.sportFacilityId = this.$route.params.id
      console.log(this.comment.state)
      axios.post('http://localhost:8080/FitnessCenter/rest/comments/', this.comment)
          .then(
              response => {
